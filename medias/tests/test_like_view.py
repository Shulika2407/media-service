...
class LikeViewsTests(MediasBaseTest):
    ...
    def test_create_duplicate_like_forbidden(self):
        ...
        self.assertTrue(any("You have already liked this post." in str(error) for error in response.data["non_field_errors"]))
    ...
    def test_list_my_likes(self):
        ...
        self.assertTrue(any(like.get('post') == self.post_by_user2.id for like in response.data))
        self.assertTrue(any(like.get('post') == self.post_by_user3.id for like in response.data))
        # Перевірка, що лайки інших користувачів не включені до списку
        self.assertFalse(any(
            like.get('user') == self.user2.id
    ...
    def test_delete_other_user_like_forbidden(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND) # Очікуємо 404
